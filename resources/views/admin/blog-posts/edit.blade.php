...
@section('title', 'Modifier l\'Article - ' . $blog_post->title)

@section('content')
    <div class="admin-page-header">
        <div>
<p style="color: var(--text-secondary); margin-top: 0.5rem;">
    Modifiez "{{ $blog_post->title }}"
</p>
        </div>
        <div class="header-actions">
            <a href="{{ route('admin.blog-posts.index') }}" class="btn-outline-admin">
                ← Retour aux articles
           <form action="{{ route('admin.blog-posts.destroy', $blog_post) }}" method="POST" style="display: inline;" 
                  onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cet article ? Cette action est irréversible.')">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn-danger" style="margin-left: 0.5rem;">
    ...
    <!-- Le reste du fichier avec $blogPost au lieu de $post -->
    ...
    @if(session('success'))
        <div style="background: rgba(76, 175, 80, 0.1); border: 1px solid #4CAF50; border-radius: 8px; padding: 1rem; margin-bottom: 2rem;">
            <h4 style="color: #4CAF50; margin-bottom: 0.5rem;">Succès !</h4>
            <p style="color: #4CAF50; margin: 0;">{{ session('success') }}</p>
        </div>
    @endif

    <div class="admin-card">
        <form action="{{ route('admin.blog-posts.update', $post) }}" method="POST" enctype="multipart/form-data" id="blogForm">
            @csrf
            @method('PUT')
       ...
       placeholder="Titre attractif pour votre article">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="excerpt" 
                                  ...
                                  rows="3"
                                  class="form-control"
                                  placeholder="Courte description qui apparaîtra dans les listes d'articles">{{ old('excerpt', $post->excerpt) }}</textarea>
                        <div class="char-counter" data-target="excerpt" data-max="500">
                            <span class="char-count">{{ strlen(old('excerpt', $post->excerpt)) }}</span>/500 caractères
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="content" 
                                  ...
                                  placeholder="Contenu détaillé de votre article...">{{ old('content', $post->content) }}</textarea>
                        <div class="char-counter" data-target="content">
                            <span class="char-count">{{ strlen(old('content', $post->content)) }}</span> caractères
                        </div>
                    </div>
                </div>
                        ...
                        <div class="image-upload-area" id="imageUploadArea">
                            @if($post->featured_image)
                                <div class="image-preview" id="imagePreview">
                                    <img id="previewImage" src="{{ $post->featured_image }}" alt="Aperçu">
                    ...
                    <div class="form-section">
                        ...
                        <div class="info-box">
                            <div class="info-item">
                                ...
                                <span class="info-value">{{ $post->created_at->format('d/m/Y à H:i') }}</span>
                            </div>
                            @if($post->updated_at->gt($post->created_at))
                            <div class="info-item">
                                <span class="info-label">Modifié le :</span>
                                <span class="info-value">{{ $post->updated_at->format('d/m/Y à H:i') }}</span>
                            </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-group">
                            ...
                            <div class="char-counter" data-target="meta_title" data-max="60">
                                <span class="char-count">{{ strlen(old('meta_title', $post->meta_title)) }}</span>/60 caractères
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="char-counter" data-target="meta_description" data-max="160">
                                <span class="char-count">{{ strlen(old('meta_description', $post->meta_description)) }}</span>/160 caractères
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <style>
    /* Styles existants de votre create... */
    .form-layout {
        display: grid;
